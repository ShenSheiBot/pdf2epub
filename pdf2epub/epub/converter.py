...
class ContentConverter:
        ...
        for md_file in markdown_dir.glob("*.md"):
            ...
            try:
                    ...
                    if current_line.strip().startswith('# ') and not current_line.strip().startswith('## '):
                        ...
                        if j < len(lines) and lines[j].strip().startswith('## ') and not lines[j].strip().startswith('### '):
                            ...
                            if similarity > 0.8 or title1.lower() == title2.lower():
                                logger.debug(f"Removing duplicate title in {md_file.name}: '## {title2}' (follows '# {title1}')")
                                new_lines.append(current_line)  # Keep level 1 heading
                                new_lines.extend(lines_between)  # Keep lines between (like author name)
                ...
                if removed_count > 0:
                    ...
                    logger.info(f"Removed {removed_count} duplicate titles from {md_file.name}")
    ...
    def copy_chapter_images(self, dest_dir: Path, compress: bool = True, 
                          max_width: int = 1600, jpeg_quality: int = 85) -> Tuple[int, Dict[str, str]]:
        """
            ...
            for img_file in images_source.iterdir():
                if img_file.suffix.lower() in image_extensions:
                    ...
                    if compress and img_file.suffix.lower() not in ['.svg', '.gif']:
                        ...
                        final_path = self._compress_and_copy_image(img_file, dest_path, max_width, jpeg_quality)
                    else:
    ...
    def _compress_and_copy_image(self, source_path: Path, dest_path_stem: Path, 
                                max_width: int, jpeg_quality: int) -> Path:
        """
    ...
    def create_xhtml_document(self, title: str, body_content: str, css_path: str = "../stylesheet.css") -> str:
        """
...
<body class="{lang_class}">
    ...
    def convert_markdown_to_chapter_html(self, markdown_path: Path, output_path: Path, 
                                        chapter_title: str, chapter_index: Optional[int] = None,
                                        subchapter_info: Optional[List] = None, 
                                        image_mapping: Optional[Dict[str, str]] = None) -> bool:
        """
        ...
        try:
            ...
            if image_mapping is not None and not isinstance(image_mapping, dict):
                logger.warning(f"Invalid image_mapping type: {type(image_mapping)}. Setting to None.")
                image_mapping = None
            ...
            if chapter_index and subchapter_info:
                html_content = self._add_subchapter_anchors(html_content, chapter_index, subchapter_info)
            ...
            logger.debug(f"image_mapping type: {type(image_mapping)}, value: {image_mapping}")
            return False
    
    def get_chapter_html_filename(self, chapter_index: int, parts_info: Optional[Dict] = None, 
                                  chapters_with_parts: Optional[set] = None) -> str:
        """
    ...
    def get_subchapter_html_file(self, chapter_index: int, subchapter_index: int, 
                                 subchapter_locations: Optional[Dict], chapter_file: str,
                                 subchapter_title: str = None) -> str:
        """
        ...
        if subchapter_locations and subchapter_title:
            ...
            if '_all_titles' in subchapter_locations:
                chapter_titles = subchapter_locations['_all_titles'].get(str(chapter_index), [])
                if chapter_titles:
                    import difflib
                    best_match = None
                    best_ratio = 0
                    ...
                    if best_match:
                        logger.debug(f"Fuzzy matched subchapter '{subchapter_title}' to '{best_match['title']}'")
    ...
    def _add_subchapter_anchors(self, html_content: str, chapter_index: int, 
                               subchapter_info: List) -> str:
        """
        ...
        if subchapter_info and isinstance(subchapter_info[0], dict):
            subchapters = [(j, info.get('title', '')) for j, info in enumerate(subchapter_info, 1)]
