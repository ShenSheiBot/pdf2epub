...
class ErrorType(Enum):
    """Categorized error types for tracking and reporting."""
    TRUNCATION = "truncation"           # Output was cut off
    ...
    CONTENT_FILTER = "content_filter"   # Content moderation rejection
...
class ProcessingTracker:
    ...
    def __init__(self, progress_path: Path, processor_name: str):
        """
    ...
    def is_unit_complete(self, unit_key: str) -> bool:
        ...
        return self.get_unit_status(unit_key) == "completed"
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        progress_path = Path(tmpdir) / "test_progress.json"
        tracker = ProcessingTracker(progress_path, "TestProcessor")

        # Record some attempts
        tracker.record_attempt("chapter_1", AttemptRecord(
            timestamp=time.time(),
            status="completed",
            model="gpt-4",
            input_tokens=1000,
        ...
        tracker.record_attempt("chapter_2.part1", AttemptRecord(
            timestamp=time.time(),
            status="failed",
            ...
            error_message="Output truncated at 1500 tokens"
        ))
        ...
        tracker.record_attempt("chapter_2.part1", AttemptRecord(
            ...
            used_fallback=True,
            fallback_reason="switched after truncation"
        ))

        # Record a split
        tracker.record_split("chapter_2", SplitRecord(
            timestamp=time.time(),
        ...
        print(tracker.get_unit_report("chapter_2.part1"))

        # Test processing plan
        plan = tracker.get_processing_plan("chapter_1")
        print(f"\nProcessing plan for chapter_1: {plan.action} - {plan.reason}")

        plan = tracker.get_processing_plan("chapter_2")
        print(f"Processing plan for chapter_2: {plan.action} - {plan.reason}")
        if plan.parts_to_process:
            print(f"  Parts to process: {plan.parts_to_process}")
