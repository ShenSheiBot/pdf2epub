...
class NGramTruncationDetector(BaseTruncationDetector):
    ...
    def __init__(
        self, min_unique_preserved_ratio: float = 0.60, allow_deduplication: bool = True
    ):
        """
    ...
    def detect(self, original: str, processed: str, **kwargs) -> Tuple[bool, str, Dict]:
        """
        ...
        if (
            ...
            return (
                False,
                f"Good preservation ({avg_unique_recall:.1%}) with deduplication ({avg_dup_ratio:.1%})",
                details,
            )
        ...
        if (
            ...
            return (
                False,
                f"Moderate preservation ({avg_unique_recall:.1%}) but mostly deduplication ({avg_dup_ratio:.1%})",
                details,
            )
        ...
        if avg_unique_recall < 0.75:
            return (
                True,
                f"Too much unique content lost (only {avg_unique_recall:.1%} preserved)",
                details,
            )
        ...
        if token_ratio < 0.7 and avg_dup_ratio < 0.5:
            return (
                True,
                f"Low token ratio ({token_ratio:.1%}) without sufficient deduplication",
                details,
            )
        ...
        return (
            False,
            f"Acceptable preservation (recall: {avg_unique_recall:.1%}, dedup: {avg_dup_ratio:.1%})",
            details,
        )
    ...
    def get_summary(self, is_truncated: bool, reason: str, details: Dict) -> str:
        ...
        if "avg_unique_recall" in details:
            summary_parts.append(
                f"Unique content recall: {details['avg_unique_recall']:.1%}"
            )
    ...
    def _get_ngram_counts(
        self, text: str, ngram_type: str = "char", k: int = 5
    ) -> Counter:
    ...
    def _calculate_duplicate_aware_metrics(
        self, original: str, polished: str, ngram_type: str = "char", k: int = 5
    ) -> Dict:
        ...
        unique_ngrams_preserved = sum(
            1 for ngram in count_in if count_out.get(ngram, 0) > 0
        )
    ...
    def _check_tail_integrity(self, text: str) -> Tuple[bool, str]:
        ...
        if (
            any(last_line.endswith(ending) for ending in sentence_endings)
            or re.search(footnote_pattern, last_line)
            or last_line.endswith(code_block_ending)
        ):
        ...
        open_brackets = (
            last_line.count("[") + last_line.count("{") + last_line.count("(")
        )
        close_brackets = (
            last_line.count("]") + last_line.count("}") + last_line.count(")")
        )
